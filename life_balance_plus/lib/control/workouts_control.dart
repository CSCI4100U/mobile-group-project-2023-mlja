...
class WorkoutControl {
  void addDummyData() {
    ...
    Exercise running = Exercise(name: 'Running', description: '');
    ...
    List<SessionLog> sessions = [
      SessionLog(
          ...
          date: DateTime.now(),
          sets: [
            ResistanceSetLog(exercise: benchPress, reps: 10, weight: 135),
            ResistanceSetLog(exercise: benchPress, reps: 8, weight: 135),
            ResistanceSetLog(exercise: benchPress, reps: 10, weight: 125),
            ResistanceSetLog(
              exercise: pushups,
              reps: 30,
            ),
      ...
      SessionLog(
          ...
          sets: [
            ...
            ResistanceSetLog(exercise: latPulldowns, reps: 10, weight: 150),
            ResistanceSetLog(exercise: latPulldowns, reps: 10, weight: 145),
            ResistanceSetLog(exercise: latPulldowns, reps: 9, weight: 140),
            CardioSetLog(exercise: running, duration: 30, avgSpeed: 5)
          ]),
    ];
  ...
  Future<List<WorkoutPlan>> getWorkoutPlans(Account account) async {
    ...
    final List localData = await db.query('workout_plans',
        where: 'accountEmail = ?', whereArgs: [account.email]);
    ...
    // Query firestore if local db is empty
      ...
      result = documents
          ...
          .toList()
          .map((plan) => WorkoutPlan.fromMap(plan))
          .toList();
    }
  ...
  Future<void> deleteWorkoutPlan(WorkoutPlan plan) async {
    final Database db = await DatabaseProvider.instance.database;
    await db.delete('workout_plans', where: 'planId = ?', whereArgs: [plan.id]);
  ...
  Future<FitnessLogs> getFitnessLogs(Account account) async {
    ...
    if (sessionMaps.isNotEmpty) {
      List setFutures = sessionMaps
          .map((sessionMap) => db.query('set_logs',
              where: 'session_log_id = ?', whereArgs: [sessionMap['id']]))
          .toList();
      ...
      for (int i = 0; i < sessionMaps.length; i++) {
        sessions[i].sets = (await setFutures[i])
            .map((setMap) {
              if (setMap['reps'] == null)
                return CardioSetLog.fromMap(setMap);
              else
                return ResistanceSetLog.fromMap(setMap);
            })
            .toList()
            .cast<SetLog>();
      }
    ...
    // Query firestore if local db is empty
      ...
      return _getFitnessLogsCloud(account);
  }
  ...
  Future<FitnessLogs> _getFitnessLogsCloud(Account account) async {
    ...
    if (sessionMaps.isNotEmpty) {
      for (int i = 0; i < sessionMaps.length; i++) {
        ...
        sessions[i].sets = querySnapshot.docs
            .map((doc) {
  ...
  Future<void> deleteSessionLog(SessionLog log) async {
    ...
    await db.delete('session_logs', where: 'id = ?', whereArgs: [log.id]);
  ...
  Future<void> updateSetLog(SetLog log, [bool updateCloud = true]) async {
    final Database db = await DatabaseProvider.instance.database;
    db.update('set_logs', log.toMap(), where: 'id = ?', whereArgs: [log.id]);
  ...
  Future<void> deleteSetLog(SetLog log) async {
    final Database db = await DatabaseProvider.instance.database;
    await db.delete('set_logs', where: 'id = ?', whereArgs: [log.id]);
